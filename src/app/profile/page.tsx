...
import { ArrowLeft, User, Shield, Lock, Save, CheckCircle, AlertCircle, LogOut } from "lucide-react";
      ...
      <div className="mx-auto max-w-2xl space-y-6">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            disabled={loggingOut}
            className="flex items-center space-x-1.5 rounded-xl border border-red-200 bg-red-50/70 px-3.5 py-1.5 text-xs font-semibold text-red-700 hover:bg-red-100/80 hover:border-red-300 active:scale-95 transition shadow-2xs cursor-pointer disabled:opacity-60"
          >
        ...
        <div className="rounded-2xl border border-slate-200/80 bg-white p-6 shadow-sm space-y-4">
          <div>
            <h3 className="text-sm font-semibold text-slate-900">Active Session & Security</h3>
            <p className="mt-0.5 text-xs text-slate-500">
              Sign out of your account on this device. Your session token will be invalidated and media vault will be locked.
            </p>
          </div>

          <div className="flex items-center justify-between pt-2 border-t border-slate-100">
            <div className="text-xs text-slate-600">
              Signed in as <span className="font-semibold text-slate-900">{profile?.email || "Member"}</span>
            </div>
            <button
              type="button"
              onClick={handleLogout}
              disabled={loggingOut}
              className="flex items-center space-x-2 rounded-xl bg-red-600 px-4 py-2 text-xs font-semibold text-white hover:bg-red-700 active:scale-95 transition shadow-sm cursor-pointer disabled:opacity-60"
            >
              <LogOut className="h-3.5 w-3.5" />
              <span>{loggingOut ? "Logging Out..." : "Sign Out of The Gallery"}</span>
            </button>
          </div>
        </div>
      </div>
